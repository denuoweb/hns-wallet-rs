...
| HNS wallet/names | adapter tranche pending consolidated gate | source includes authenticated loopback RPC configuration, strict HTTP/JSON parsing, atomic snapshot restoration, mempool-bound point reads, and workflow recovery | source includes epoch-bound checkpoint rewind, ordered spender evidence, and split current/proof evidence revalidation | concrete adapter source pinned to node RPC v1 commit `74f7ae36`; no multi-process regtest run, canonical NameState decoder, or dedicated name-role scan | no resource measurement/audit | value runtime unavailable; names watch-only |
