...
| ABI/host | new `production_followup_` required/sorted/bounded/canonical name-disclosure and exact-host-retention cases are source-only and unexecuted; earlier account-join/control passes predate the new shape | source includes exact sequencing/correlation/authority/approval state plus updated private/public schemas, structural vectors, and runtime-invalid ordering/hash vectors; strict old/new permission summaries reject one another | earlier restart evidence predates this change | no browser/mobile ABI-v2 adoption, trusted rendering, platform E2E, verifier, launcher, or generated binding | no resource measurement/audit | product integration pending; private control dispatch only; browser/value unavailable |
