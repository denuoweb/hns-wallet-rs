...
and two service `production_followup_` regressions. They cover the live exact
...
followed by additive read permission, exact bounded name scope, minimized
...
cargo test --locked --offline \
  -p hns-wallet-hns -p hns-wallet-provider -p hns-wallet-service \
