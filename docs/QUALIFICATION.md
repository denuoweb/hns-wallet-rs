...
| Fixed-price Shakedex | current focused `hns_shakedex` PASS on NVMe: 1 aggregate unit passed, 0 failed, 4 filtered; 1 restart integration passed, 0 failed, 0 filtered; exact-lock/Denuo/board filter PASS on NVMe: 1 passed, 0 failed, 3 filtered; prior immutable-V2 listing/gate filter: 3 passed, 0 failed; consolidated evidence pending | encrypted parent-plan/board CAS plus protected monotonic seller-key allocation; new buyer-fulfillment/seller-recovery child binds exact structural plan, source/funding evidence and reservations, approval/revision, final signed bytes/fee quote, and pre-submit fence; persisted quote evidence is revalidated without restoring snapshot authority | source state machine retains protected reservations and uses runtime-owned transaction/all-input-spender evidence for authorized-row recovery, mempool, confirmation, conflict, rollback, and same-byte rebroadcast; no executed multi-process restart/reorg evidence, durable script-FINALIZE child, or signed-terminal reservation release | no regtest/live-Denuo/provider/UI E2E | no audit | all Shakedex and dependent HNS Shakedex-funding/value/fee gates remain false |
...
durable script-controlled FINALIZE, and evidence-backed release of signed-
workflow reservations remain pending; every related release gate stays `false`.
