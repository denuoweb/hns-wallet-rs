...
| Provider core | prior focused `canonical_provider_account_join` PASS on NVMe: 5 passed, 0 failed, 31 filtered across FFI, host, provider, and service; prior focused `production_tranche_` persistent-control PASS: 1 passed, 0 failed, 7 filtered; six new `production_next_` source regressions are not compiled or executed; consolidated gate pending | source gives provider state, runtime control, and the exact HNS selector one locked store/key authority; rejects unlocked, different-Arc, zero-account, or malformed-row composition; rechecks the selected singleton across restart; retains encrypted grants/tombstones while dropping process-local authorities/approvals/replays; prior focused file-backed reopen evidence passed | n/a | checked-in executable remains the exact five-control dispatcher; library source adds the exact seven-method account composition but no synchronized HNS chain reads or installed-browser E2E | no audit | product integration pending; value/browser unavailable |
