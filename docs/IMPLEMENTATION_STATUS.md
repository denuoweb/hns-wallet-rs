...
| Provider | exact 43-name vocabulary, all-HNS namespace enforcement, secure origin, opaque authority registry, authority-validated permission/tombstone snapshots, singleton persisted account binding, generation-CAS-bound single-approval `hns_requestAccounts` join, runtime-selection-rechecked `hns_accounts`, additive account-scoped read grants, bounded exact known-name scopes, typed capability snapshot, ephemeral approvals/replay/rates, forbidden methods; checked-in existing-database control dispatcher exposes exactly five controls; library compositions expose the account join alone or the join plus synchronized balance/history/receive/name reads; public HNS results use explicit bounded camelCase/decimal-string/lowercase-hex projections and omit internal snapshot/proof/resource/owner/derivation evidence | execute the new source-only provider/service regressions; trusted backend/product construction, published engine authority adapter, browser-native transport, complete trusted approval UI, installed restart/regtest/adversarial qualification; value and module methods remain unavailable |
...
This commit adds `production_followup_` HNS, provider, and service regressions
for the synchronized read composition, exact live binding and durable fence,
no store-lock/backend overlap, restart/stale/account/lock failure, scoped name
authority, minimized public shapes, numeric/list bounds, namespace/permission
