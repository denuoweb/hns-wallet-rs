...
| Handshake Provider API | implemented source, unavailable | The private service now has bounded persisted account scopes and a generation-bound, runtime-selected `hns_requestAccounts`/`hns_accounts` contract that fails closed on selector downgrade. A concrete `HnsWalletRuntime` selector and method adapter, trusted browser transport/UI, events, installed-product E2E, restart qualification, and reviewed provider gates remain unavailable. |
