...
| Handshake Provider API | implemented source, unavailable | The checked-in existing-database subprocess now starts locked with one shared provider/runtime store key, implements ABI status/unlock/lock, and exposes only capabilities/status/permission read-or-revoke/lock after unlock; generic permission creation is absent because no permission-bearing runtime method is active. The private service also has bounded persisted account scopes and a generation-bound, runtime-selected `hns_requestAccounts`/`hns_accounts` contract that fails closed on selector downgrade, but the subprocess advertises neither account method. Its focused encrypted reopen regression passes. A concrete `HnsWalletRuntime` selector/read adapter, trusted browser transport/UI, events, installed-product restart/E2E qualification, and reviewed availability gates remain unavailable. |
